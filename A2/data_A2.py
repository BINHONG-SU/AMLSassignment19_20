...
class data_A2:
    def train(self):    #get the training data
        sum = 5000
        ...
        for i in range(0,sum):
            ...
            img_gray_re = img_gray[20:158,20:100]   #Crop image, only keep the lower part
            img[i] = img_gray_re.flatten()
        X_raw = np.array(img)

        labels = pd.read_csv('././datasets/celeba/labels.csv')   #Read the labels
        Y_raw = labels['smiling']
        ...
        n_componets = 90    #Use PCA to reduce the demensianlity of input data
        pca = PCA(n_components=n_componets,random_state=99)
        pca.fit(X_sta)
        print('A2_pca variance',pca.explained_variance_ratio_.sum())    #Check the information kept after dimensionality reduction
    ...
    def test(self):  #get the testing dataset
        ...
        for i in range(0,sum):
            ...
            img_gray = cv2.cvtColor(img_,cv2.COLOR_RGB2GRAY)    #Convert to the gray value
            img_gray_re = img_gray[20:158,20:100]   #Crop image
            img[i] = img_gray_re.flatten()
        X_raw = np.array(img)
