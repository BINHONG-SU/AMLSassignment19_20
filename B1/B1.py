...
class B1:
    ...
    def train(self,x_train, y_train,x_val,y_val):
        try_C=np.logspace(-10, -5, 6, endpoint=True, base=2)  #Set the hyperparameters need to be trainedS
        iter_C = len(try_C)
        val_acc_max = 0
        for i in range(0,iter_C):
            clf = LinearSVC(C = try_C[i], max_iter =999999)   #Use a LinearSVC model
            ...
            if val_acc > val_acc_max:   #save the best estimator
                val_acc_max = val_acc
                self.model = clf
        print('B1 best estimator',self.model)
